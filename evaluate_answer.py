...
def eval_result(predict_file, cal_f1=True, topk=-1):
    ...
    detailed_eval_file = predict_file.replace('answer.jsonl', eval_name)
    # Load results
    acc_list = []
    hit_list = []
    f1_list = []
    ...
    with open(predict_file, 'r') as f, open(detailed_eval_file, 'w') as f2:
        for line in f:
            ...
            if cal_f1:
                ...
                hit_list.append(hit)
                f2.write(json.dumps(
                    {'id': id, 'prediction': prediction, 'truth': answer, 'acc': acc, 'hit': hit, 'f1': f1_score,
                     'precission': precision_score, 'recall': recall_score}, ensure_ascii=False) + '\n')
            else:
                ...
                hit_list.append(hit)
                f2.write(json.dumps(
                    {'id': id, 'prediction': prediction, 'truth': answer, 'acc': acc, 'hit': hit}, ensure_ascii=False) + '\n')
    ...
    eval_result_path = predict_file.replace('answer.json', result_name)
